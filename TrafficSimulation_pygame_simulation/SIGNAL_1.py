...
from subscribe_1 import mqttLoop, mqttMessage
import subscribe_1
import os

# Default values of signal timerscd
...
def repeat():
    ...
    while (signals[currentGreen].green > 0):  # while the timer of current green signal is not zero
        updateValues()
        ################################################################################
        if subscribe_1.stopSignal :
            time.sleep(15)
            subscribe_1.stopSignal = False
        ################################################################################
        time.sleep(1)
...
class Main:
    ...
    pygame.display.set_caption("SIGNAL 1")
    ...
    ##################################################################################################
    myThread = threading.Thread(target=mqttLoop, args=())
    ...
    while True:
        ...
        for i in range(0, noOfSignals):  # display signal and set timer according to current status: green, yello, or red
            ...
            ### to call signal 0
            if count == 0:
                print("relay:", i+1, "\tsignal_timer:", signals[i].green)
                client.publish('signal1/relay1', signals[i].green)
                print("sended:", signals[i].green)
            count = count + 1

            ### to call rest of the signals
            if (i == currentGreen):
                # print(signals[i].green, i)
                print("signal_no:",i,"\tsignal_timer:",signals[i].green)
                current_value = i
                if current_value != previous_value:
                    print("previous value:",previous_value)
                    print("current value:",current_value)
                    _signal = f"signal1/relay{i+1}"
                    print(_signal)
                    print("relay: ",i+1 , "\tsignal_timer:", signals[i].green)
                    client.publish(_signal, signals[i].green)
                    print("sended:", signals[i].green)

                    previous_value = current_value
                # if previous_value
                # if previous_value != current_value:
                    print("get into loop:",signals[i].green)

                # print(signals[i].green) #current green signal time in second
                if (currentYellow == 1):
